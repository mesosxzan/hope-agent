...
| `server` | `bindAddr` (e.g. `127.0.0.1:8420` vs `0.0.0.0:8420`), `apiKey`, `publicBaseUrl`, `defaultTimezone` (IANA name e.g. `"Asia/Shanghai"`, used when `UserConfig.timezone` is unset and the host zone is UTC — the typical Docker scenario; `null` = fall back to UTC). **Read responses redact `apiKey` to `"[REDACTED]"`** so the bearer token isn't echoed back on every overview; writes still flow through. | Network exposure, requires app restart |
