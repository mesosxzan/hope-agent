...
| Cron 定时任务 | `NoopEventSink` + `chat:stream_delta` EventBus | 定时触发的对话复用同一个 noop sink；`ChatSource::Cron.broadcasts_to_user_ui()=true`，故同时上主 `chat:stream_delta` bus 供 `CronSessionViewer` 的 `useCronStream` 实时流式输出。无 `ChannelStreamSink`，故无 `channel:stream_delta` 第二路径双渲染；`turn_id=None` 故 `chat:turn_started` 不发，前端挂载时靠 `get_session_stream_state` 探活并补种占位。最终投递结果仍由 Cron delivery 处理 |
...
- **`NoopEventSink`**（定义在 `crates/ha-core/src/chat_engine/types.rs`）— 丢弃 per-call sink 事件。HTTP 模式、Cron 定时任务、subagent fork-and-forget 等入口共用此 sink。HTTP 与 Cron 的实时浏览器 / 前端流式输出由 Chat Engine 的 `chat:stream_delta` EventBus 双写路径推到 `/ws/events`（`broadcasts_to_user_ui()=true`）；subagent 不上 bus（`broadcasts_to_user_ui()=false`，子会话无 UI 对端）。
